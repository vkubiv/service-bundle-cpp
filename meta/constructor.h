...
T* ConstructTypeImpl(TInjector & injector, std::index_sequence<Ns...>)
{
    return new T(any_type_t<TInjector, T, Ns>(injector)...);
}

template<class T, class TInjector>
std::unique_ptr<T> ConstructType(TInjector & injector)
{
    constexpr auto size = get_ctor_size<T>(
        std::make_index_sequence<BOOST_DI_CFG_CTOR_LIMIT_SIZE>{});

    return std::unique_ptr<T>(ConstructTypeImpl<T>(injector, std::make_index_sequence<size>()));
}
}
}
