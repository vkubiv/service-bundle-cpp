...
namespace meta
{
    ...
    {
        enum { value = TypeIndexImpl<T, TypeList...>::value + 1 };
    };
    ...
    struct TypeIndex<T, TypeList<List...>> {
        enum { value = TypeIndexImpl<T, List...>::value };
    };
    ...
    {
        enum { value = 1 + ListSize <TypeList<List...>>::value };
    };
    ...
    template<typename T, int N, int M = 0> struct TypeListGet;
    ...
    struct TypeListGet< TypeList<T, TT...>, N, M > {
        ...
        using Type = std::conditional_t< N == M, T, typename TypeListGet< TypeList<TT...>, N, M + 1 >::Type >;
    };

    template<int N, int M> struct TypeListGet< TypeList<>, N, M > { using Type = void; };

    template<int N> struct TypeListGet< TypeList<>, N, 0 > {};
