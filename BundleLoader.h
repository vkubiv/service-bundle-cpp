...
public:
    ...
    boost::future<BundleRef<Bundle>> activate(Externals&&... externals)
    {
        ...
        return bundle->activate().then([bundle](auto result) -> BundleRef<Bundle>
        {
            result.get();
...
private:
    ...
    int setExternal(IBundleInstaceImpl & bundle, External&& externals)
    {
        ...
        static_assert(externalsIndex != meta::InvalidTypeIndex && externalsIndex >= 0, "Type is not listed on bundle externals list");

        bundle.setExternalsRef(externalsIndex, std::make_unique<ExternalsRefT<External>>(std::move(externals)));
        return 0;
    }
